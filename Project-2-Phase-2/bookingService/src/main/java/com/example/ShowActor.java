...
public class ShowActor extends AbstractBehavior<ShowActor.Command> {
    ...
    public static final EntityTypeKey<ShowActor.Command> TypeKey = EntityTypeKey.create(ShowActor.Command.class,
            "SimpleCounterEntity");
    ...
    public final record Booking(Integer id, Integer user_id, Integer show_id, Integer seats_booked)
            implements CborSerializable {
    }
    ...
    public final record DeleteUserBooking(Integer user_id, Integer show_id, ActorRef<Response> replyTo,
            ActorRef<WorkerActor.Command> workerReply) implements Command {
    }
    ...
    public final record GetTheatre(ActorRef<WorkerActor.Command> workerRef, Integer i, Integer theatre_id,
            ActorRef<WorkerActor.Showlist> replyTo) implements Command {
    }

    private ShowActor(ActorContext<Command> context, Integer id, Integer theatre_id, String title, Integer price,
    ...
    private Behavior<Command> onAddBooking(AddBooking message) {
        ...
        if (seats_booked > -1 && seats_booked <= this.seats_available
                && WalletService.walletOperation(user_id, amount, "debit", http)) {
    ...
    private Behavior<Command> onGetTheatre(GetTheatre message) {
        ...
        message.workerRef()
                .tell(new WorkerActor.TheatreOfShow(status,
                        new Show(this.id, this.theatre_id, this.title, this.price, this.seats_available), message.i,
                        message.replyTo));
        return this;
    }
