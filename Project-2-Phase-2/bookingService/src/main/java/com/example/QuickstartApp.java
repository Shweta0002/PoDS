            ...
            sharding.init(
                    Entity.of(ShowActor.TypeKey,
                            entityContext -> {
                                ...
                                return ShowActor.create(Integer.parseInt(entityId), show.theatre_id(), show.title(),
                                        show.price(), show.seats_available());
                            }));
            if (port == 8083) {
                for (String line : shows) {
                    ...
                    EntityRef<ShowActor.Command> ref2 = sharding.entityRefFor(ShowActor.TypeKey,
                            Integer.toString(show_id));
                ...
                // context.getLog().info("ShardRegion actors started on node with port {}",
                // port);
                ActorRef<BookingRegistry.Command> bookingRegistryActor = context
                        .spawn(BookingRegistry.create(router, showActors), "UserRegistry");
                ...
                for (int i = 0; i < 50; i++) {
                    ActorRef<WorkerActor.Command> worker = context.spawn(WorkerActor.create(showActors),
                            "worker" + replica * 50 + i);
