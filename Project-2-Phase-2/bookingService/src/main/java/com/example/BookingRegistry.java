...
public class BookingRegistry extends AbstractBehavior<BookingRegistry.Command> {
  ...
  public final record DeleteUserBooking(Integer user_id, Integer show_id, ActorRef<ShowActor.Response> replyTo)
      implements Command {
  }

  public ActorRef<WorkerActor.Command> router;

  private BookingRegistry(ActorContext<Command> context, ActorRef<WorkerActor.Command> router,
      Map<Integer, EntityRef<ShowActor.Command>> showActors) {
    super(context);
    this.router = router;
  ...
  public static Behavior<Command> create(ActorRef<WorkerActor.Command> router,
      Map<Integer, EntityRef<ShowActor.Command>> showActors) {
